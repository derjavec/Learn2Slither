...
def run_session(cfg: dict, q_table: dict,
                visual: bool = False, step: bool = False) -> dict:
    """
...
def run_model_sessions(model: dict, sessions: int,
                       visual: bool, step: bool) -> dict:
    """
    ...
    stats = {
        "avg_snake_len": sum(e["snake_len"] for e in ep_res) / sessions,
        "avg_moves": sum(e["moves"] for e in ep_res) / sessions,
        "avg_green_apples": sum(e["green_apples"] for e in ep_res) / sessions,
        "avg_red_apples": sum(e["red_apples"] for e in ep_res) / sessions,
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model_id", type=int,
                        help="Specify the ID of the model to run")
    parser.add_argument("--sessions", type=int,
                        help="Number of sessions to run per model")
    parser.add_argument("--step_by_step",
                        action="store_true", help="Run model step by step")
    parser.add_argument("--visual", action="store_true",
                        help="Visualize board in terminal")
    return parser


def main():
    """
    Main function to run one or multiple
    trained Slither models, collect statistics,
    ...
    for idx, model in enumerate(history):
        print(f"\n--- Running Model {idx} ---")
        stats = run_model_sessions(model, sessions,
                                   args.visual, args.step_by_step)
