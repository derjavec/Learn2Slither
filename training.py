...
from utils.history\
    import add_to_history, load_models, rewrite_history_with_updates
...
def run_sessions(q_table: dict, config: dict, n_sessions: int) -> dict:
    ...
    for episode in range(n_sessions):
        ...
        while not board.done:
            ...
            decision = take_decision_training(q_table, state,
                                              config['e_greedy'],
                                              board.snake_dir)
        ...
        print(f"episode {episode} | snake len {len(board.snake_pos)}\
              | moves {movements} | q_table {len(q_table)}")
...
def main():
    ...
    - If add_sessions is None, runs new sessions
        and adds a new model to history.
    - If add_sessions is specified, adds sessions
        to existing model(s) and updates history.
    """
    ...
    for m in models:
        ...
        updated_models.append({"id": m["id"], "config": m["config"],
                               "q_table": q_new})
